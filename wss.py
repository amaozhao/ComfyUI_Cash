...
async def getHistoryPrompt(prompt_id, type_a=""):
    ...
    try:
        if prompt_id in result:
            ...
            if status.get("completed", False):
                ...
                result_data.append(
                    {"type": "str", "k": "ok", "v": "0", "text": "completed状态不对"}
                )
        else:
            ...
            if is_set:
                return
            result_data.append(
                {"type": "str", "k": "ok", "v": "0", "text": "prompt_id没有找到"}
            )
        response_status = 200
    except Exception as e:
        print_exception_in_chinese(e)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        try:
            ...
            print_exception_in_chinese(e)
            result_data.append(
                {"type": "str", "k": "ok", "v": "0", "text": "json异常的信息"}
            )
            response_status = 400
        except Exception as e:
            print_exception_in_chinese(e)
                ...
                websocket_queue.append(
                    {
                        ...
                        "data": {
                            ...
                            "data": {"jilu_id": jilu_id, "msg": "作品工作流找不到了"},
                        },
                    }
                )
        pass
            ...
            websocket_queue.append(
                {
                    ...
                    "data": {
                        ...
                        "data": {
                            "client_id": new_client_w_id,
                            "queue_remaining": message_json["data"]["status"][
                                "exec_info"
                            ]["queue_remaining"],
                        },
                    },
        ...
        try:
            async for message in websocket:
                if type(message) != bytes:
                    ...
                    if conn_identifier == 1:
                        await server1_receive_messages(
                            websocket, message_type, message_dict
                        )
                    elif conn_identifier == 2:
                        await server2_receive_messages(
                            websocket, message_type, message_dict
                        )
        except json.JSONDecodeError as e:
            print_exception_in_chinese(e)
...
async def process_json_elements(json_data, prompt_data, workflow, jilu_id):
        ...
        websocket_queue.appendleft(
            {
                ...
                "data": {
                    "type": "crystools.prompt_error",
                    "data": {"jilu_id": jilu_id, "msg": "发送指令失败1"},
                },
            }
        )
        return {"code": 0, "jilu_id": jilu_id}
    except Exception as e:
        print_exception_in_chinese(e)
        websocket_queue.appendleft(
            {
                ...
                "data": {
                    "type": "crystools.prompt_error",
                    "data": {"jilu_id": jilu_id, "msg": "发送指令失败2"},
                },
            }
        )
        return {"code": 0, "jilu_id": jilu_id}
    ...
    async def print_item(key, value):
        try:
            ...
            if (
                ...
                and "crf" in json_data[key]["inputs"]
            ):
            ...
            websocket_queue.appendleft(
                {
                    ...
                    "data": {
                        "type": "crystools.prompt_error",
                        "data": {"jilu_id": jilu_id, "msg": "发送指令失败3"},
                    },
                }
            )
    ...
    try:
        ...
        try:
            ...
            print_exception_in_chinese(e)
        task_queue_3[result["prompt_id"]] = {"jilu_id": jilu_id}
        return {"code": 1, "prompt_id": result["prompt_id"]}
    except Exception as e:
        print_exception_in_chinese(e)
        websocket_queue.appendleft(
            {
        ...
        return {"code": 0, "prompt_id": jilu_id}
...
def print_thread_status():
    while True:
        ...
        for thread in threading.enumerate():
            print(
                f"线程名: {thread.name}, 线程ID: {thread.ident}, 活动状态: {thread.is_alive()}"
            )
        time.sleep(5)
